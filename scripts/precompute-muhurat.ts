...
import {calendarMonthSsgPriority,muhuratCityMonthSsgPriority,muhuratMonthSsgPriority,muhuratYearSsgPriority} from "../lib/static-seo-routes";
...
function collectTargets():Target[]{
  const raw=[
    ...
    ...calendarMonthSsgPriority.map(item=>({city:item.city,year:Number(item.year),month:Number(item.month)})),
    ...muhuratYearSsgPriority.flatMap(item=>Array.from({length:12},(_,index)=>({city:"mumbai",year:Number(item.year),month:index+1}))),
  ];
