...
export default async function CityYearCalendar({params}:{params:Promise<{city:string;year:string}>}){
  ...
  return <main><Header city={city}/><div className="page-shell internal-visual internal-calendar">
    ...
    <section className="wide-panel"><h2 className="page-title" style={{fontSize:32}}>Festivals in {year}</h2><p className="page-subtitle">{qualityContent.festivalBody}</p>{festivals.length?<div className="city-directory">{festivals.map(item=><Link href={`/festivals/${item.slug}/${year}/${city.slug}`} key={item.slug}><small>{item.date}</small><strong>{item.name}</strong><span>Open local Panchang context</span></Link>)}</div>:<p className="page-subtitle">No validated festival records are currently stored for {year}; Panchvani does not synthesize missing festival dates.</p>}</section>
