...
export default async function MuhuratPage({params}:{params:Promise<{event:string;year:string;month:string}>}){
  ...
  return <main><Header city={city}/><div className="page-shell internal-visual internal-muhurat">
    ...
    {top?<section className="wide-panel"><div className="seo-copy"><small>TOP PRACTICAL FIT · MUMBAI BASELINE</small><h2>{top.date} · {top.planning.grade} · {top.planning.score}/100</h2><p>{top.reasons.join(" · ")}. The longest clean block on this screened date is {top.planning.longestWindowMinutes} minutes, with {top.planning.totalCleanMinutes} total clean minutes in the current timing model.</p><p>{top.recommendedWindows[0]?`First local planning window: ${top.recommendedWindows[0].start}–${top.recommendedWindows[0].end}.`:"No clean favorable daytime window remains after exclusions."}</p></div></section>:null}
