...
export async function generateMetadata({params}:{params:Promise<{event:string;year:string;month:string}>}):Promise<Metadata>{
  ...
  return {title:`${rule.title} ${year} — Ranked Dates & Local Windows`,description:`Ranked ${rule.title.toLowerCase()} dates with Panchvani Planning Score and filtered local auspicious reference windows for ${p.month}/${year}.`,alternates:{canonical:`/muhurat/${p.event}/${year}/${p.month}`},robots:robotsFor(isYearlyMuhuratIndexable(p.event,year))};
}
...
export default async function MuhuratPage({params}:{params:Promise<{event:string;year:string;month:string}>}){
  ...
  return <main><Header city={city}/><div className="page-shell internal-visual internal-muhurat">
    <div className="breadcrumbs"><Link href={muhuratYearPath(p.event,year)}>{rule.title} {year}</Link> / {p.month}</div>
    ...
    <section className="wide-panel">
      <div className="seo-copy">
        ...
        <h2>{seo.headline}</h2><p>{seo.overview}</p><p>{seo.rankingInsight}</p><p>{seo.timingInsight}</p><p>{seo.alternatives}</p>
      </div>
    ...
    {top?<section className="wide-panel"><div className="seo-copy"><small>TOP PLANNING FIT · MUMBAI BASELINE</small><h2>{top.date} · {top.planning.grade} · {top.planning.score}/100</h2><p>{top.reasons.join(" · ")}. The longest clean block is {top.planning.longestWindowMinutes} minutes, with {top.planning.totalCleanMinutes} unique clean minutes across favorable daytime periods.</p><p>{top.recommendedWindows[0]?`First available reference window: ${top.recommendedWindows[0].start}–${top.recommendedWindows[0].end}.`:"No clean favorable daytime window remains after exclusions."}</p></div></section>:null}
    ...
    <div className="seo-copy"><h2>How the Panchvani Planning Score works</h2><p>A date first has to qualify through the event's Tithi and Nakshatra rule profile. The score then compares those qualified dates using practical local availability: the longest uninterrupted clean block, total unique clean minutes, the number of distinct favorable timing sources and whether an Abhijit block remains after exclusions.</p><p>The continuity target is event-specific so a longer ceremony is not evaluated like a short purchase activity. Overlapping favorable periods are counted only once in the clean-minute total. Rahu Kalam, Yamaganda and Gulika never add points and are removed before scoring.</p><p>This ranking is a Panchvani planning aid, not a claim that one date is universally more religiously auspicious than another. Personal horoscope compatibility, Tara Bala, Lagna and sampradaya-specific ritual constraints remain outside this general shortlist.</p></div>

    <div className="pill-links"><Link href={muhuratYearPath(p.event,year)}>Full {year} {rule.title}</Link></div>
