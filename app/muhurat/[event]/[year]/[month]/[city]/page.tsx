...
export default async function Page({params}:{params:Promise<{event:string;year:string;month:string;city:string}>}){
  ...
  return <main><Header city={city}/><div className="page-shell internal-visual internal-muhurat">
    ...
    <p className="page-subtitle">{muhuratScreeningStatement} Planning Scores compare the clean-time availability produced by this screen.</p>
    ...
    <section className="wide-panel"><div className="seo-copy"><small>ADDITIONAL FACTORS</small><h2>Checks outside the current screen</h2><ul>{muhuratExcludedFactors.map(item=><li key={item}>{item}</li>)}</ul><p>These factors remain outside the current screen. <strong>Reference conventions:</strong> {muhuratBenchmarkSources.map((source,index)=><span key={source.label}>{index?" · ":""}{source.label}</span>)}</p></div></section>
