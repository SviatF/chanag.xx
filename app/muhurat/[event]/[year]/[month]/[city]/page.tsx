...
import {isMuhuratMonthIndexable,robotsFor} from "@/lib/seo-policy";
...
export async function generateMetadata({params}:{params:Promise<{event:string;year:string;month:string;city:string}>}):Promise<Metadata>{
  ...
  return {title:`${rule.title} in ${city.name} — Candidate Dates ${p.month}/${year}`,description:`${rule.title} candidate dates in ${city.name} using Panchvani's Tithi + Nakshatra screening profile, with local planning windows and explicit limitations.`,alternates:{canonical:`/muhurat/${p.event}/${year}/${p.month}/${city.slug}`},robots:robotsFor(isMuhuratMonthIndexable(p.event,year,month,city.slug))};
}
...
export default async function Page({params}:{params:Promise<{event:string;year:string;month:string;city:string}>}){
  ...
  return <main><Header city={city}/><div className="page-shell internal-visual internal-muhurat">
    ...
    {top?<section className="wide-panel"><div className="seo-copy"><small>TOP PRACTICAL FIT · {city.name.toUpperCase()}</small><h2>{top.date} · {top.planning.grade} · {top.planning.score}/100</h2><p>{top.reasons.join(" · ")}. The longest uninterrupted clean block is {top.planning.longestWindowMinutes} minutes and the total clean-time supply is {top.planning.totalCleanMinutes} minutes.</p><p>{top.recommendedWindows[0]?`First local planning window: ${top.recommendedWindows[0].start}–${top.recommendedWindows[0].end}.`:"No clean favorable daytime window remains after exclusions."}</p></div></section>:null}
