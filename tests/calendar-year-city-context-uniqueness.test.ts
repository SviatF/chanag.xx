...
function snapshot(month:number):Panchang{
  ...
  return {
    date,weekday:"Thursday",tithi:["Panchami","Ashtami","Ekadashi","Purnima"][month%4],tithiEnd:"14:20",tithiEndDate:date,paksha:month%2?"Shukla":"Krishna",nakshatra:["Rohini","Swati","Pushya","Revati"][month%4],nakshatraEnd:"17:45",nakshatraEndDate:date,nakshatraPada:2,rashi:["Mesha","Vrishabha","Mithuna","Karka"][month%4],solarRashi:["Makara","Mesha","Karka","Tula"][month%4],yoga:"Siddhi",karana:"Bava",
    sunrise:`06:${String(10+Math.abs(6-month)).padStart(2,"0")}`,sunset:`18:${String(25+Math.abs(6-month)).padStart(2,"0")}`,moonrise:"19:04",moonriseDate:date,moonset:"05:20",moonsetDate:date,moonIllumination:(month*8)%100,
...
describe("Yearly calendar city semantic dedup layer",()=>{
  it("keeps every phase-one city distinct after city names and numbers are stripped",()=>{
    ...
    const festivals=[
      {name:"Makar Sankranti",slug:"makar-sankranti",date:"2026-01-14"},
    ...
    const values=phase1PriorityCities.map(slug=>{
      ...
      const context=buildCalendarYearCityContext(city,2026,snapshots,festivals);
      const text=[context.title,context.body,context.seasonalTitle,context.seasonalBody,context.lunarTitle,context.lunarBody,context.festivalTitle,context.festivalBody,...context.facts.map(item=>`${item.label} ${item.value} ${item.note??""}`)].join(" ");
