...
function dataset():GoldRateDataset{
  ...
  return {updatedAt:new Date().toISOString(),validatedSince:new Date(Date.now()-15*86400000).toISOString(),updateFrequency:"daily",source:{name:"Test source",url:"https://example.com"},national:{rates:{"24k":10020,"22k":9220,"18k":7520},history},cities:{mumbai:{rates:{"24k":10030,"22k":9230,"18k":7530},history}}};
}
...
describe("gold rate launch guardrails",()=>{
  ...
  it("opens only after validation, freshness and city history pass",()=>{process.env.GOLD_RATE_PUBLIC_ENABLED="true";process.env.GOLD_RATE_INDEXING_ENABLED="true";process.env.GOLD_RATE_INDEX_CITIES="mumbai";expect(goldRateGate(dataset()).ready).toBe(true);});
