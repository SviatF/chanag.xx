...
const vehicleOpeningDetails:Record<string,string>={
  Monday:"For a Monday-opening purchase year, the comparison lens is week-start execution: shortlist dates are read against dealer confirmation, finance paperwork, registration preparation and handover steps that can begin at the start of a working cycle. The useful signal is whether the strongest months keep that early-week operational cadence or push the best screened choices toward later weekdays.",
  Tuesday:"For a Tuesday-opening purchase year, the planning frame begins after the first working-day reset. Read the strongest months as a mid-process procurement sequence: document checks, payment readiness, insurance coordination and delivery timing are compared against candidate weekdays to see whether the annual shortlist remains early-week or develops a later-cycle handover rhythm.",
  Wednesday:"For a Wednesday-opening purchase year, the operational lens is explicitly midweek. Think of the shortlist as a procurement chain that can move from stock confirmation and finance approval into registration preparation before the weekend boundary. The strongest months are therefore compared for midweek continuity: whether leading dates repeat Wednesday-side timing, migrate toward Tuesday or Thursday, and whether clean blocks support a compact same-week handover plan rather than a weekend-adjacent fallback pattern.",
  Thursday:"For a Thursday-opening purchase year, the operational lens is late-week clearance. The shortlist is interpreted around a different handover sequence: paperwork and payment may need to clear before a weekend interruption, while delivery or registration steps may spill into the following working cycle. Strong months are compared for late-week pressure versus next-week flexibility, with special attention to whether leading dates cluster near Thursday and Friday or move decisively back toward Monday-to-Wednesday execution.",
  Friday:"For a Friday-opening purchase year, the annual planning map starts at the weekend threshold. Candidate months are compared by whether their best dates favour immediate pre-weekend handover or defer practical execution into the next working cycle, creating a distinct delivery-and-registration cadence across the year.",
  Saturday:"For a Saturday-opening purchase year, the baseline is weekend-first rather than working-week-first. The useful annual distinction is how quickly shortlisted dates return to weekday execution and whether the strongest months support weekend-adjacent delivery or require a separate registration and paperwork cycle.",
  Sunday:"For a Sunday-opening purchase year, the year begins on a non-working-week boundary. The comparison therefore emphasises the transition from weekend planning into weekday execution, separating candidate months whose leading dates cluster near that boundary from months that sit deeper inside the working week.",
};
...
export function buildMuhuratYearHighSimilarityContext(summary:YearlyMuhuratSummary):MuhuratYearHighSimilarityContext{
  ...
  const vehicleDetail=summary.event==="vehicle-purchase"?(vehicleOpeningDetails[jan1]??""):"";
  const eventBody=`${profile.body} ${opening.body} ${vehicleDetail} In this ${leapText} frame, the result is a ${chronology}. ${firstActive?`${firstActive.name} opens the ${profile.decisionNoun}`:`No month opens the ${profile.decisionNoun}`}; ${lastActive?`${lastActive.name} closes it.`:"the year stays closed."} ${leadMonth?`${leadMonth.name} is the deepest decision month.`:"There is no density leader."} ${fallbackMonth?`${fallbackMonth.name} acts as the ${profile.fallbackNoun}.`:"There is no second active fallback month."}`;
