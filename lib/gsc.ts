...
export function getGscConnectionStatus(){
  ...
  return {
    configured:Boolean(required.GOOGLE_SERVICE_ACCOUNT_EMAIL&&required.GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY),
    required,
...
export function gscCheckpointWindow(shippedAt:string,days:14|28|56){const shipped=new Date(`${shippedAt.slice(0,10)}T00:00:00Z`);if(Number.isNaN(shipped.getTime()))throw new Error("Invalid shippedAt date for GSC checkpoint.");const preEnd=shift(shipped,-1);const preStart=shift(shipped,-days);const postStart=new Date(shipped);const postEnd=shift(shipped,days-1);return {preStart:iso(preStart),preEnd:iso(preEnd),postStart:iso(postStart),postEnd:iso(postEnd)};}
export function isGscCheckpointReady(shippedAt:string,days:14|28|56,asOf=new Date()){const {postEnd}=gscCheckpointWindow(shippedAt,days);const finalAvailableEnd=iso(shift(asOf,-2));return finalAvailableEnd>=postEnd;}
...
export async function getGscOutcomeComparisons(requests:GscOutcomeRequest[]):Promise<Record<string,GscOutcomeComparison>>{
  ...
  await Promise.all(requests.map(async request=>{const window=gscCheckpointWindow(request.shippedAt,request.days);const filters:SearchAnalyticsFilter[]=[{dimension:"query",operator:"equals",expression:request.query}];const [preRaw,postRaw]=await Promise.all([query(token,status.siteUrl,{startDate:window.preStart,endDate:window.preEnd,dimensions:["query","page"],rowLimit:5000,filters}),query(token,status.siteUrl,{startDate:window.postStart,endDate:window.postEnd,dimensions:["query","page"],rowLimit:5000,filters})]);output[`${request.key}:${request.days}`]={query:request.query,pre:aggregateOutcomeWindow(window.preStart,window.preEnd,preRaw.rows),post:aggregateOutcomeWindow(window.postStart,window.postEnd,postRaw.rows)};}));
  return output;
}
...
export async function getGscTrafficSnapshot():Promise<GscTrafficSnapshot>{
  ...
  const today=new Date();const end=shift(today,-2);const start=shift(end,-27);const previousEnd=shift(start,-1);const previousStart=shift(previousEnd,-27);
  const startDate=iso(start);const endDate=iso(end);const previousStartDate=iso(previousStart);const previousEndDate=iso(previousEnd);const token=await accessToken();
  const [currentRaw,previousRaw,pagesRaw,queriesRaw,queryPagesRaw,dailyRaw]=await Promise.all([query(token,status.siteUrl,{startDate,endDate,rowLimit:1}),query(token,status.siteUrl,{startDate:previousStartDate,endDate:previousEndDate,rowLimit:1}),query(token,status.siteUrl,{startDate,endDate,dimensions:["page"],rowLimit:5000}),query(token,status.siteUrl,{startDate,endDate,dimensions:["query"],rowLimit:10000}),query(token,status.siteUrl,{startDate,endDate,dimensions:["query","page"],rowLimit:25000}),query(token,status.siteUrl,{startDate,endDate,dimensions:["date"],rowLimit:1000})]);
  return {siteUrl:status.siteUrl,startDate,endDate,previousStartDate,previousEndDate,current:summary(currentRaw.rows),previous:summary(previousRaw.rows),pages:pagesRaw.rows??[],queries:queriesRaw.rows??[],queryPages:queryPagesRaw.rows??[],daily:(dailyRaw.rows??[]).sort((a,b)=>(a.keys?.[0]??"").localeCompare(b.keys?.[0]??""))};
}
...
async function inspectIndexUrl(token:string,siteUrl:string,inspectionUrl:string):Promise<GscIndexInspection>{
  const response=await fetch("https://searchconsole.googleapis.com/v1/urlInspection/index:inspect",{method:"POST",headers:{authorization:`Bearer ${token}`,"content-type":"application/json"},body:JSON.stringify({inspectionUrl,siteUrl,languageCode:"en-US"}),cache:"no-store"});
  if(!response.ok){const text=await response.text();throw new Error(`Search Console URL Inspection failed for ${inspectionUrl} (${response.status}): ${text.slice(0,220)}`);}
  const json=await response.json() as RawInspectionResponse;const row=json.inspectionResult?.indexStatusResult??{};
...
export async function getGscUrlInspections(urls:string[],concurrency=4):Promise<{rows:GscIndexInspection[];errors:string[]}>{
  const unique=[...new Set(urls.map(url=>url.trim()).filter(Boolean))];if(!unique.length)return {rows:[],errors:[]};
  const status=getGscConnectionStatus();if(!status.configured)throw new Error("Google Search Console service account is not configured.");const token=await accessToken();const rows:GscIndexInspection[]=[];const errors:string[]=[];const width=Math.max(1,Math.min(8,Math.floor(concurrency)));
  for(let index=0;index<unique.length;index+=width){const chunk=unique.slice(index,index+width);const results=await Promise.allSettled(chunk.map(url=>inspectIndexUrl(token,status.siteUrl,url)));results.forEach((result,offset)=>{if(result.status==="fulfilled")rows.push(result.value);else errors.push(result.reason instanceof Error?result.reason.message:`URL Inspection failed for ${chunk[offset]}`);});}
  return {rows,errors};
}
