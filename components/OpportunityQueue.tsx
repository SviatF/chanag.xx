...
export default function OpportunityQueue({opportunities,records,storageConfigured,storageError}:Props){
  ...
  const learning=useMemo(()=>buildSeoLearningLibrary(records),[records]);
  ...
  const rows=useMemo(()=>{
    ...
    const score=(row:QueueRow)=>row.opportunity?prioritizeOpportunityWithLearning(row.opportunity,learning).adjustedScore:0;
    return active.sort((a,b)=>order[a.record.stage]-order[b.record.stage]||score(b)-score(a)||a.key.localeCompare(b.key));
  },[opportunities,records,learning]);
  ...
  return <>
    ...
    <section className="admin-panel">
      ...
      <p className="admin-muted">Every implementation carries a frozen change hypothesis and attribution evidence. Historical measured implementations also feed a conservative learning adjustment back into new opportunity priority; raw GSC demand remains the base score and learning never changes lifecycle state automatically.</p>
      <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Opportunity</th><th>Demand + learning</th><th>Lifecycle</th><th>Outcome intelligence</th><th>Action intelligence + build brief</th><th>Implementation evidence</th><th>Owner / note</th><th>Lifecycle action</th></tr></thead><tbody>
        {rows.map(row=>{
          const item=row.opportunity;const context=item?opportunityContext(item):row.record.context;const outcome=latestOutcomeCheckpoint(row.record);const plan=buildSeoActionPlan({opportunity:item,record:row.record});const brief=buildSeoExecutionBrief({opportunity:item,record:row.record});const nextCheckpoint=nextOutcomeCheckpoint(row.record);const draft=draftFor(row.key,row.record);const next=nextOpportunityStages(row.record.stage);const persisted=Boolean(records[row.key]);const activeImpl=activeImplementation(row.record);const implementationReady=implementationReadyForShip(activeImpl);const learned=item?prioritizeOpportunityWithLearning(item,learning):null;
          return <tr key={row.key}>
            ...
            <td>{item&&learned?<><b>{learned.adjustedScore}/100</b>{learned.delta!==0?<span className={`admin-badge ${learned.delta>0?"active":"hold"}`}>Learning {signed(learned.delta)}</span>:null}<small>Raw {item.score}/100 · {n(item.impressions)} impr. · {n(item.clicks)} clicks</small><small>pos {item.position.toFixed(1)} · CTR {pct(item.ctr)}</small><small>{learned.pattern?`${learned.pattern.replaceAll("_"," ")} · ${learned.signal} · confidence ${learned.confidence}%`:learned.rationale}</small>{learned.pattern?<small>{learned.rationale}</small>:null}</>:<small>No current 28-day discovery row</small>}</td>
