...
export default function SeoQueriesClient(){
  ...
  return <>
    ...
    <div className={styles.toolbar}><div className={styles.filters}>{filters.map(([value,label])=><button key={value} className={filter===value?styles.active:""} onClick={()=>choose(value)}>{label}</button>)}</div><button className={styles.refresh} title="Only this button performs a network refresh; filters and search are local." onClick={()=>void load(true)} disabled={loading}><RefreshCw size={14}/> {loading?"Оновлюю…":"Оновити дані"}</button></div>
    <div className={styles.toolbar}><label className={styles.search}><Search size={14}/><input value={search} onChange={event=>{setSearch(event.target.value);setPage(1);}} placeholder="Пошук по query або URL…"/></label><span className={styles.note}>{payload?`GSC final data · ${payload.dataset.current28d.startDate} → ${payload.dataset.current28d.endDate} · browser cache 5 хв · GSC cache 15 хв${payload.telemetry?` · ${payload.telemetry.cache} · ${payload.telemetry.apiCalls} API · ${payload.telemetry.upstreamSubrequests} subreq · ${payload.telemetry.latencyMs} ms`:""}`:""}</span></div>
