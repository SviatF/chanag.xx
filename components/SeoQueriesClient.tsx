...
function matchesFilter(row:QueryIntelligence,filter:Filter){
  if(filter==="ALL")return true;if(filter==="3PLUS")return row.impressions>=3;if(filter==="TOP3")return row.position<=3;if(filter==="TOP10")return row.position<=10;if(filter==="TOP20")return row.position<=20;if(filter==="TOP50")return row.position<=50;
  return row.statuses.includes(filter as QueryStatus);
}
...
export default function SeoQueriesClient(){
  ...
  return <>
    ...
    <div className={styles.toolbar}><div className={styles.filters}>{filters.map(([value,label])=><button key={value} className={filter===value?styles.active:""} onClick={()=>choose(value)}>{label}</button>)}</div><button className={styles.refresh} title="Manual refresh is throttled to protect API/runtime cost." onClick={()=>void load(true)} disabled={loading}><RefreshCw size={14}/> {loading?"Оновлюю…":"Оновити дані"}</button></div>
    <div className={styles.toolbar}><label className={styles.search}><Search size={14}/><input value={search} onChange={event=>{setSearch(event.target.value);setPage(1);}} placeholder="Пошук по query або URL…"/></label><span className={styles.note}>{payload?`GSC final data · ${payload.dataset.current28d.startDate} → ${payload.dataset.current28d.endDate} · browser cache 60 хв · backend cache 6 год`:""}</span></div>
