...
function ResultCards({slug,data}:{slug:ExpandedToolSlug;data:Awaited<ReturnType<typeof getPanchang>>}){
  ...
  return <div className="data-grid">
    ...
    <div className="data-card"><small>Amanta lunar month</small><strong>{data.hinduMonth}</strong></div>
...
function Explanation({slug}:{slug:ExpandedToolSlug}){
  ...
  if(slug==="hindu-month-finder")return <><h2>How the Hindu month is determined</h2><p>This tool uses the Amanta convention: the lunar month is derived from the sidereal solar sign around the previous new moon. Purnimanta and regional calendars can use a different month label for the same civil date.</p></>;
  return <><h2>One-date Panchang lookup</h2><p>Select any supported Indian city and date to open the calculated Tithi, Nakshatra, Yoga, Karana and local solar timings, with a direct link to the full daily Panchang.</p></>;
}
