  ...
  return <div className={styles.shell}>
    ...
    <section className={styles.section}>
      ...
      <div className={styles.taskList}>{visiblePrioritized.length?visiblePrioritized.map(page=>{const globalIndex=prioritized.findIndex(item=>item.url===page.url);return <article className={styles.taskCard} key={page.url}>
        ...
        <div className={styles.taskInner}>
          ...
          <div className={styles.taskFooter}><div className={styles.evidence}><span>28d evidence</span><span>{page.priority}</span><span>{page.cannibalizationRisk==="NONE"?"no cannibalization":"intent conflict"}</span></div><button className={styles.observeButton} onClick={()=>setSelected(page)}><CheckCircle2 size={13}/>Зміну реалізовано → revalidate</button></div>
        </div>
    ...
    <section className={styles.section}>
      ...
      {observationTasks.length?<div className={styles.observationGrid}>{observationTasks.map(task=>{const current=currentMetricForTask(task,pages);const ready=new Date(task.verifyAt).getTime()<=Date.now();const verdict=ready?evaluateTask(task,current):null;return <article className={`${styles.observationCard} ${ready?styles.observationReady:""}`} key={task.id}>
        ...
        <div className={styles.observationFooter}><div className={styles.commit}>{task.revalidatedAt?`Revalidated ${fmtDateTime(task.revalidatedAt)} · ${task.revalidatedPath}${task.commitSha?` · Commit ${task.commitSha}`:""}`:task.commitSha?`Commit ${task.commitSha}`:"10-day observation cycle"}</div>{ready?<button className={styles.observeButton} onClick={()=>void closeTask(task,verdict??"low_data")}>Закрити цикл</button>:<span className={styles.lockPill}><Clock3 size={11}/>не чіпати</span>}</div>
